...
import {
  ...
  type GjcTaskLifecycleBoundary,
} from "../acp/AcpRuntimeModel.ts";
import { makeAcpNativeLoggerFactory } from "../acp/AcpNativeLogging.ts";
import {
  applyGjcAcpModelSelection,
  currentGjcModelIdFromSessionSetup,
  makeGjcAcpRuntime,
...
import { type GjcAdapterShape } from "../Services/GjcAdapter.ts";
import { type EventNdjsonLogger, makeEventNdjsonLogger } from "./EventNdjsonLogger.ts";

const encodeUnknownJsonStringExit = Schema.encodeUnknownExit(Schema.fromJsonString(Schema.Unknown));

const PROVIDER = ProviderDriverKind.make("gjc");
const GJC_RESUME_VERSION = 1 as const;
...
export interface GjcAdapterLiveOptions {
...
interface GjcSessionContext {
  ...
  /** Raw GJC task-batch lifecycle boundaries already emitted in this session. */
  readonly taskLifecycles: Map<string, GjcTaskLifecycleState>;
  stopped: boolean;
}
...
const resolveNotificationTurnId = (ctx: GjcSessionContext): TurnId | undefined => ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: GjcSessionContext): TurnId | undefined => ctx.activeTurnId;

const resolveSessionCallbackTurnId = (
  sessions: ReadonlyMap<ThreadId, GjcSessionContext>,
  threadId: ThreadId,
): TurnId | undefined => {
...
function parseGjcResume(raw: unknown): { sessionId: string } | undefined {
  if (!isRecord(raw)) return undefined;
  if (raw.schemaVersion !== GJC_RESUME_VERSION) return undefined;
...
export function gjcPromptSettlementBelongsToContext(input: {
...
export function makeGjcAdapter(gjcSettings: GjcSettings, options?: GjcAdapterLiveOptions) {
  return Effect.gen(function* () {
    const boundInstanceId = options?.instanceId ?? ProviderInstanceId.make("gjc");
    ...
    const sessions = new Map<ThreadId, GjcSessionContext>();
      ...
      Effect.mapError(
        (cause) =>
          new ProviderAdapterRequestError({
            ...
            detail: "Failed to generate GJC runtime identifier.",
            cause,
          }),
      ),
    );
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process GJC ACP callback.",
              cause,
            }),
        ),
      );
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = gjcPromptSettlementBelongsToContext({
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("Failed to write native GJC notification log.", {
            cause,
            threadId,
            method,
          }),
    ...
    const emitTaskLifecycleFromToolCall = (
      ctx: GjcSessionContext,
      ...
      Effect.gen(function* () {
        const boundary = readGjcTaskLifecycleBoundary(rawPayload);
        if (!boundary) {
          return;
        }
        const existing = ctx.taskLifecycles.get(toolCallId);
        ...
        if (boundary.kind === "started") {
          ...
          yield* offerRuntimeEvent({
            ...
            payload: {
              taskId,
              description: boundary.agentName,
              agentName: boundary.agentName,
              taskType: "subagent",
              requestedTaskIds: boundary.requestedTaskIds,
        ...
        if (boundary.kind === "completed") {
          const partialReason = boundary.reason;
          yield* offerRuntimeEvent({
            type: "task.completed",
            ...stamp,
            provider: PROVIDER,
            ...
            payload: {
              taskId,
              status: partialReason ? "failed" : "completed",
        ...
        yield* offerRuntimeEvent({
          ...
          payload: {
            taskId,
            status: boundary.reason === "no_allocated_agent_ids" ? "stopped" : "failed",
            agentName: existing.agentName,
            taskType: "subagent",
            requestedTaskIds: boundary.requestedTaskIds,
            subagentCount: boundary.subagentCount,
    ...
    const requireSession = (
      threadId: ThreadId,
    ): Effect.Effect<GjcSessionContext, ProviderAdapterSessionNotFoundError> => {
      const ctx = sessions.get(threadId);
      if (!ctx || ctx.stopped) {
        return Effect.fail(
    ...
    const stopSessionInternal = (ctx: GjcSessionContext) =>
      Effect.gen(function* () {
    ...
    const startSession: GjcAdapterShape["startSession"] = (input) =>
      withThreadLock(
        input.threadId,
        Effect.gen(function* () {
          ...
          const resumeSessionId = parseGjcResume(input.resumeCursor)?.sessionId;
          ...
          // Steer support: GJC's ACP agent rejects a second session/prompt
          // while one is active unless the steering queue is enabled. Advertise
          // "all" immediately after startup so model selection observes the
          // same session configuration as subsequent prompts. Best-effort:
          // older GJC builds without the option keep the default (steer then
          // degrades to a conflict error, which surfaces as a typed adapter
          // error).
          yield* acp.setConfigOption("steeringMode", "all").pipe(Effect.ignore);

          const requestedStartModelId = gjcModelSelection?.model
            ? resolveGjcAcpBaseModelId(gjcModelSelection.model)
            : undefined;
          const boundModelId = yield* applyGjcAcpModelSelection({
            runtime: acp,
            currentModelId: currentGjcModelIdFromSessionSetup(started.sessionSetupResult),
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(boundModelId ? { model: resolveGjcAcpBaseModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
              schemaVersion: GJC_RESUME_VERSION,
          ...
          const nf = yield* Stream.runDrain(
              ...
              Effect.gen(function* () {
                ...
                switch (event._tag) {
                  ...
                  case "ToolCallUpdated":
                    ...
                    yield* emitTaskLifecycleFromToolCall(
                      ctx,
                      notificationTurnId,
                      stamp,
            ...
            Effect.catch((cause) =>
              Effect.logError("Failed to process GJC runtime notification.", { cause }),
            ),
            Effect.forkChild,
          );
          ...
          yield* offerRuntimeEvent({
            ...
            provider: PROVIDER,
            threadId: input.threadId,
            payload: { state: "ready", reason: "GJC ACP session ready" },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
            ...(yield* makeEventStamp()),
    ...
    const sendTurn: GjcAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const requestedTurnModelId = turnModelSelection?.model
                ? resolveGjcAcpBaseModelId(turnModelSelection.model)
                : undefined;
              const currentModelId = yield* applyGjcAcpModelSelection({
                runtime: ctx.acp,
              ...
              const promptParts = yield* makePromptParts(text, input.attachments ?? []);
              ...
              const displayModel = currentModelId
                ? resolveGjcAcpBaseModelId(currentModelId)
                : undefined;
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "GJC prompt was interrupted during preparation.",
                });
              }
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "GJC prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  {
                    errorMessage: "GJC session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "GJC session changed before the turn completed.",
                });
              }
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "GJC prompt request failed.",
                }),
              );
    ...
    const steerTurn: NonNullable<GjcAdapterShape["steerTurn"]> = (input) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(input.threadId);
        ...
        if (!text && attachments.length === 0) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "steerTurn",
            issue: "Steer message must include non-empty text or attachments.",
          });
        }
        // Steer is text-only on the wire (GJC's SDK turn.steer carries text;
        // image content is not supported). Reject attachments with a typed
        // validation error before any turn-state checks.
        if (attachments.length > 0) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "steerTurn",
            issue: "steer supports text only",
          });
        }
        const liveTurnId = ctx.activeTurnId ?? ctx.session.activeTurnId;
        if (input.turnId === undefined || input.turnId !== liveTurnId) {
          return yield* new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "session/prompt",
            detail: `Cannot steer stale turn '${input.turnId ?? "missing"}'; live turn is '${liveTurnId ?? "none"}'.`,
          });
        }

        // Reserve a prompt slot before preparing the prompt. Preparation reads
        // attachments in the filesystem and yields; without this reservation,
        // the host can observe the original prompt as settled and detach the
        // active turn before the steer reaches ACP.
        ctx.promptsInFlight += 1;
        let promptSlotHeld = true;
        const releasePromptSlot = Effect.sync(() => {
          if (!promptSlotHeld) {
            return;
          }
          promptSlotHeld = false;
          ctx.promptsInFlight = Math.max(0, ctx.promptsInFlight - 1);
        });
        const preparedPrompt = yield* Effect.exit(makePromptParts(text || undefined, attachments));
        if (Exit.isFailure(preparedPrompt)) {
          yield* releasePromptSlot;
          return yield* Effect.failCause(preparedPrompt.cause);
        }

        // Rebind to the live context after preparation. An interrupt, session
        // replacement, or turn settlement during the preparation yield makes
        // this steer stale; release the reserved slot and report it instead of
        // dispatching against a detached context.
        const liveCtx = sessions.get(input.threadId);
        const liveTurnIdAfterPreparation = liveCtx
        ...
        if (
          ...
          input.turnId !== liveTurnIdAfterPreparation
        ) {
          yield* releasePromptSlot;
          return yield* new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "session/prompt",
            detail: `Cannot steer stale turn '${input.turnId ?? "missing"}'; live turn is '${liveTurnIdAfterPreparation ?? "none"}'.`,
          });
        }

        yield* liveCtx.acp.steer({ prompt: preparedPrompt.value }).pipe(
          Effect.mapError((error) =>
            mapAcpToAdapterError(PROVIDER, input.threadId, "session/prompt", error),
          ),
          Effect.asVoid,
          Effect.ensuring(releasePromptSlot),
        );
      });

    const interruptTurn: GjcAdapterShape["interruptTurn"] = (threadId, turnId) =>
      Effect.gen(function* () {
    ...
    const respondToRequest: GjcAdapterShape["respondToRequest"] = (threadId, requestId, decision) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
    ...
    const respondToUserInput: GjcAdapterShape["respondToUserInput"] = (
      threadId,
      requestId,
      answers,
    ) =>
    ...
    const readThread: GjcAdapterShape["readThread"] = (threadId) =>
      Effect.gen(function* () {
        const ctx = yield* requireSession(threadId);
        return { threadId, turns: ctx.turns };
      });

    const rollbackThread: GjcAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "GJC ACP sessions do not support provider-side rollback yet.",
        });
      });

    const stopSession: GjcAdapterShape["stopSession"] = (threadId) =>
      withThreadLock(
        threadId,
        Effect.gen(function* () {
          const ctx = yield* requireSession(threadId);
    ...
    const listSessions: GjcAdapterShape["listSessions"] = () =>
      Effect.sync(() => Array.from(sessions.values(), (c) => ({ ...c.session })));

    const hasSession: GjcAdapterShape["hasSession"] = (threadId) =>
      Effect.sync(() => {
        const c = sessions.get(threadId);
        return c !== undefined && !c.stopped;
      });

    const stopAll: GjcAdapterShape["stopAll"] = () =>
    ...
    return {
      provider: PROVIDER,
      capabilities: { sessionModelSwitch: "in-session", steer: "supported" },
      startSession,
      sendTurn,
      steerTurn,
      interruptTurn,
